...
function clanleave(msg) {
    clan[profile[msg.author.id].clan].membersnb -= 1; (clan[profile[msg.author.id].clan].members).splice((clan[profile[msg.author.id].clan].members).indexOf(msg.author.id), (clan[profile[msg.author.id].clan].members).indexOf(msg.author.id)); fs.writeFile(`./data/clan.json`, JSON.stringify(clan), err => { if (err) throw err; });
    profile[msg.author.id].clan = false; profile[msg.author.id].clanexperience = 0; fs.writeFile(`./data/profile.json`, JSON.stringify(profile), err => { if (err) throw err; });
}
function clanjoin(name, msg) {
    clan[name].membersnb += 1; (clan[name].members).push(msg.author.id); fs.writeFile(`./data/clan.json`, JSON.stringify(clan), err => { if (err) throw err; });
    profile[msg.author.id].clanexperience = 0; profile[msg.author.id].clan = name; fs.writeFile(`./data/profile.json`, JSON.stringify(profile), err => { if (err) throw err; });
}
function clandelete(name, msg) {
    for (i in clan[name].members) {
        var userKick = client.users.cache.get((clan[name].members)[i]);
        profile[((clan[name].members)[i])].clanexperience = 0; profile[((clan[name].members)[i])].clan = false; fs.writeFile(`./data/profile.json`, JSON.stringify(profile), err => { if (err) throw err; });
        if (userKick) {
...
function clankick(msg, userid) {
    clan[profile[msg.author.id].clan].membersnb -= 1; clan[profile[msg.author.id].clan].members.splice((clan[profile[msg.author.id].clan].members).indexOf(userid), (clan[profile[msg.author.id].clan].members).indexOf(userid)); fs.writeFile(`./data/clan.json`, JSON.stringify(clan), err => { if (err) throw err; });
    profile[userid].clanexperience = 0; profile[userid].clan = false; fs.writeFile(`./data/profile.json`, JSON.stringify(profile), err => { if (err) throw err; });
...
function claninvite(name, userid, msg) {
    clan[name].membersnb += 1; (clan[name].members).push(userid); fs.writeFile(`./data/clan.json`, JSON.stringify(clan), err => { if (err) throw err; });
...
function levelUpMessage(msg, level) {
    ...
    if (profile[msg.author.id].clan != false) {
        clan[profile[msg.author.id].clan].experience += levelUpClanXp; fs.writeFile(`./data/clan.json`, JSON.stringify(clan), err => { if (err) throw err; });
        profile[msg.author.id].clanexperience += levelUpClanXp; fs.writeFile(`./data/profile.json`, JSON.stringify(profile), err => { if (err) throw err; });
    }
...
function clanLevelUpMessage(name, msg, level) {
    ...
    switch (level) {
        ...
        case 3: var xpBoost = "× 1.5"; break;
        case 4: var xpBoost = "× 1.7"; break;
        case 5: var xpBoost = "× 2.0"; break;
        case 6: var xpBoost = "× 2.2"; break;
        case 7: var xpBoost = "× 2.5"; break;
        case 8: var xpBoost = "× 2.7"; break;
        case 9: var xpBoost = "× 3.0"; break;
        case 10: var xpBoost = "× 3.5"; break;
        default: var xpBoost = "× 1.0"; break;
    }
...
client.on('message', msg => {
    ...
    if (!userCooldown.has(msg.author.id)) {
        xpWon = Math.floor(Math.random() * (30 - 20 + 1)) + 20;
        ...
        if (!profile[msg.author.id]) {
            ...
            if (profile[msg.author.id].clan != false) {
                ...
                profile[msg.author.id].clanexperience += xpWon; fs.writeFile(`./data/profile.json`, JSON.stringify(profile), err => { if (err) throw err; });
                // update clan level
                ...
                switch (clan[profile[msg.author.id].clan].level) {
                    ...
                    case 3: xpWon = xpWon * 1.5; break;
                    case 4: xpWon = xpWon * 1.7; break;
                    case 5: xpWon = xpWon * 2.0; break;
                    case 6: xpWon = xpWon * 2.2; break;
                    case 7: xpWon = xpWon * 2.5; break;
                    case 8: xpWon = xpWon * 2.7; break;
                    case 9: xpWon = xpWon * 3.0; break;
                    case 10: xpWon = xpWon * 3.5; break;
                    default: xpWon = xpWon * 3.5; break;
                }
...
client.login(`ODAyOTc2NDk0MDkxNjMyNjcw.YA3EIg.1YPcxqanYsVxmjs9bXLjv2ZmZlY`);